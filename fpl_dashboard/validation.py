        ...
        rows.append(
            {
                ...
                "Timestamp source": item.timestamp_source or item.timestamp_column or "",
                "Timestamp confidence": item.timestamp_detection_confidence,
                "Total rows": item.row_count,
                "Valid timestamps": item.timestamp_valid_count,
                "Unique timestamps": item.timestamp_unique_count,
                "First timestamp": item.first_timestamp.strftime("%Y-%m-%d %H:%M") if item.first_timestamp is not None else "",
                "Last timestamp": item.last_timestamp.strftime("%Y-%m-%d %H:%M") if item.last_timestamp is not None else "",
                ...
                "Interval confidence": item.interval_detection_confidence,
                "Interval notes": item.interval_detection_note,
                "Value unit": item.interval_value_unit_note,
                "Unit confidence": item.interval_value_unit_confidence,
